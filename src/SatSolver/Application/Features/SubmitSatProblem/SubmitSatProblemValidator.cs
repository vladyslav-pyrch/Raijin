...
public class SubmitSatProblemValidator : AbstractValidator<SubmitSatProblemCommand>
{
    public SubmitSatProblemValidator()
    {
        RuleFor(command => command.Dimacs)
            .NotEmpty()
            .Custom(ValidateDimacs);
    }

    private static void ValidateDimacs(string dimacs, ValidationContext<SubmitSatProblemCommand> context)
    {
        if (dimacs is null)
            return;
    ...
    private static bool TryParseProblemLine(
        (string Line, int Index) problem,
        ValidationContext<SubmitSatProblemCommand> context,
        out int numVars,
        out int numClauses)
    {
        numVars = 0;
    ...
    private static bool ValidateClauseCount(
    ...
    private static void ValidateClauseLiterals(
        string[] tokens,
        int lineNumber,
        int numVars,
        ValidationContext<SubmitSatProblemCommand> context)
