...
public sealed class EdgeColoringSetProblemInstance(
    IValidator<EdgeColoringInstanceDto>? validator = null
) : ISetProblemInstanceExtension
{
    public string ProblemType => ProblemTypes.EdgeColoringProblem;

    public Result<Instance> CreateInstance(InstanceDto instanceDto)
    {
        if (instanceDto is not EdgeColoringInstanceDto dto)
            throw new ArgumentException(
                $"Invalid instance DTO type. Expected {typeof(EdgeColoringInstanceDto)}, got {instanceDto.GetType()}");
        ...
        List<ValidationError> duplicateErrors = duplicateVertices
            .Select(id => new ValidationError(
                nameof(EdgeColoringInstanceDto.Vertices),
        ...
        foreach (EdgeDto edgeDto in dto.Edges)
        {
            ...
            if (!uValid)
                edgeErrors.Add(new ValidationError($"{nameof(EdgeColoringInstanceDto.Edges)}[{edgeIndex}].{nameof(EdgeDto.U)}",
                    $"Vertex '{edgeDto.U}' does not exist in the vertex list."));
            if (!vValid)
                edgeErrors.Add(new ValidationError($"{nameof(EdgeColoringInstanceDto.Edges)}[{edgeIndex}].{nameof(EdgeDto.V)}",
        ...
        List<ValidationError> selfLoopErrors = edges
            ...
            .Select(t => new ValidationError(
                $"{nameof(EdgeColoringInstanceDto.Edges)}[{t.i}]",
        ...
        return new EdgeColoringInstance(graph, dto.ColorCount);
    }
}

public record EdgeColoringInstanceDto(
    IReadOnlyList<string> Vertices,
    IReadOnlyList<EdgeDto> Edges,
    int ColorCount
) : InstanceDto
{
    [JsonIgnore] public override string ProblemType => ProblemTypes.EdgeColoringProblem;
}

public record EdgeDto(string U, string V);
