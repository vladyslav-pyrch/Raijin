...
public sealed record SetEdgeColoringProblemInstanceCommand(
    Guid ProblemId,
    EdgeColoringInstanceDto Instance
) : IRequest;

public sealed class SetEdgeColoringProblemInstanceHandler(
    IProblemRepository problemRepository,
    IUnitOfWork unitOfWork
) : IRequestHandler<SetEdgeColoringProblemInstanceCommand>
{
    public async Task<Result> Handle(
        SetEdgeColoringProblemInstanceCommand request,
        CancellationToken cancellationToken)
    {
        ...
        List<ValidationError> duplicateVertexErrors = request.Instance.Vertices
            ...
            .Select(g => new ValidationError(
                $"{nameof(request.Instance)}.{nameof(EdgeColoringInstanceDto.Vertices)}",
        ...
        List<ValidationError> duplicateEdgeLabelErrors = request.Instance.Edges
            ...
            .Where(g => g.Count() > 1)
            .Select(g => new ValidationError(
                $"{nameof(request.Instance)}.{nameof(EdgeColoringInstanceDto.Edges)}",
        ...
        foreach (EdgeColoringEdgeDto edgeDto in request.Instance.Edges)
        {
            ...
            if (!uValid)
                edgeErrors.Add(new ValidationError(
                    $"{nameof(request.Instance)}.{nameof(EdgeColoringInstanceDto.Edges)}[{edgeIndex}].{nameof(EdgeColoringEdgeDto.U)}",
                    $"Vertex '{edgeDto.U}' does not exist in the vertex list."));
            if (!vValid)
                edgeErrors.Add(new ValidationError(
                    $"{nameof(request.Instance)}.{nameof(EdgeColoringInstanceDto.Edges)}[{edgeIndex}].{nameof(EdgeColoringEdgeDto.V)}",
        ...
        List<ValidationError> selfLoopErrors = edges
            ...
            .Select(t => new ValidationError(
                $"{nameof(request.Instance)}.{nameof(EdgeColoringInstanceDto.Edges)}[{t.i}]",
        ...
        problem.SetInstance(new EdgeColoringInstance(new Graph(vertices, edges), request.Instance.ColorCount));
...
public sealed record EdgeColoringInstanceDto(
    ...
    IReadOnlyList<EdgeColoringEdgeDto> Edges,
    int ColorCount
);

public sealed record EdgeColoringEdgeDto(string Label, string U, string V);
...
public sealed class EdgeColoringInstanceDtoValidator : AbstractValidator<EdgeColoringInstanceDto>
{
    ...
    public EdgeColoringInstanceDtoValidator()
    {
        ...
        RuleFor(dto => dto.ColorCount)
            .GreaterThan(0)
            .WithMessage("Colour count must be greater than 0.");
...
public sealed class SetEdgeColoringProblemInstanceValidator : AbstractValidator<SetEdgeColoringProblemInstanceCommand>
{
    public SetEdgeColoringProblemInstanceValidator()
    {
        RuleFor(c => c.ProblemId).NotEmpty();
        RuleFor(c => c.Instance)
            .NotNull()
            .SetValidator(new EdgeColoringInstanceDtoValidator());
    }
}
