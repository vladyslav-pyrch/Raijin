...
- **Major services:**
  - `ApiGateway`: Entry point for HTTP traffic, minimal logic.
  - `CombinatoricsService` & `IdentityService`: Each has `Api/`, `Application/`, `Domain/`, `Infrastructure/`, and `MigrationWorker/` subfolders, following Clean Architecture.
...
- **Frontend-backend:** REST via ApiGateway
...
**For more details, see `CLAUDE.md` and feature folders in each service.**
